IsoPlot <- function(get, name, only.sig.iso = FALSE, ylim = NULL, xlab = "Time", ylab = "Expression value", points = TRUE, cex.main = 3,
                    cex.legend = 1.5) {
  ...
  if (is.null(ylim)) {
    ylims <- c(min(data, na.rm = TRUE), max(data, na.rm = TRUE))
  }
  ...
  for (i in 1:ncol) {
    ...
    ploti(
      dati = data[, group == codeg[i]], ti = time[group == codeg[i]], op = op, ylim = ylims, ylab = ylab, xlab = xlab, yaxt = yaxt,
      main = codeg[i], points, cex.main
    )
  }
  plot.new()
