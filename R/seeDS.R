seeDS <- function(get, rsq = 0.4, cluster.all = TRUE, plot.mDSG = FALSE, k = 6, cluster.method = "hclust", k.mclust = FALSE, ...) {
  # cluster.all=TRUE, cluster with significant isoforms of all the genome.  cluster.all=FALSE, cluster with DETs of DSGs
  ...
  if (plot.mDSG) {
    ...
    H1 <- see.genes(data.clust1,
      edesign = design$edesign, cluster.method = cluster.method, k.mclust = k.mclust, k = k, item = "Isoforms",
      ...
    )
  }
