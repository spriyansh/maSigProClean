"PlotProfiles" <- function(data, cond, cex.axis = 0.5, ylim = NULL, repvect, main = NULL, sub = NULL, color.mode = "rainbow", item = NULL) {
  pos.vline <- repvect[1:length(repvect)] - c(0, repvect[1:(length(repvect) - 1)])
  ...
  if (!is.vector(data)) {
    ...
    plot(x = c(1:n), y = data[1, ], type = "l", col = 1, ylim = ylim, ylab = "expression value", xlab = " ", main = nom, xaxt = "n")
    axis(1, at = 1:n, labels = substr(cond, 1, 26), cex.axis = cex.axis, las = 2)
    ...
    axis(1, at = 1:n, labels = cond, cex.axis = cex.axis, las = 2)
    abline(v = pos.vline, col = "light gray")
  }
}
