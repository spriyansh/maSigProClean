"PlotGroups" <- function(data, edesign = NULL, time = edesign[, 1], groups = edesign[, c(3:ncol(edesign))], repvect = edesign[, 2], show.lines = TRUE,
                         show.fit = FALSE, dis = NULL, step.method = "backward", min.obs = 2, alfa = 0.05, nvar.correction = FALSE, summary.mode = "median",
                         groups.vector = NULL, main = NULL, sub = NULL, xlab = "Time", ylab = "Expression value", item = NULL, ylim = NULL, pch = 21, col = NULL,
  ...
  plot(
    x = time, y = yy, pch = pch, xlab = xlab, ylab = ylab, main = main, xaxt = "n", sub = sub, ylim = ylim, xlim = xlim, col = color2,
    ...
  )
  arg <- list(...)
  axis(1, at = unique(abcissa), labels = unique(abcissa), lwd = 1, cex.axis = arg$cex.axis)
  if (show.fit) {
    rm <- matrix(yy, nrow = 1, ncol = length(yy))
    rownames(rm) <- c("ratio medio")
    colnames(rm) <- rownames(dis)
    fit.y <- T.fit(rm, design = dis, step.method = step.method, min.obs = min.obs, alfa = alfa, nvar.correction = nvar.correction)
  ...
  for (i in 1:ncol(groups)) {
    ...
    if ((show.fit) && !is.null(betas)) {
      ...
      curve(a[1] + a[2] * x + a[3] * (x^2) + a[4] * (x^3) + a[5] * (x^4) + a[6] * (x^5) + a[7] * (x^5),
  ...
  if (legend) {
    legend(max(abcissa, na.rm = TRUE) * 1.02, ylim[1],
      legend = codeg, text.col = color1, col = color1, lty = lty.legend, yjust = 0,
